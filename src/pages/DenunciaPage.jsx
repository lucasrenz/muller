...
export default function DenunciaPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const payload = {
        anonimo: !identificar,
        vinculo_grupo: form.vinculo_grupo || null,
  ...
  return (
    <>
      <style>{`
        ...
        .denuncia-option-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(8.5rem, 1fr)); gap: 0.75rem; }
        .denuncia-option-button { min-height: 3rem; border-radius: 0.75rem; border: 1px solid rgba(137,115,98,0.22); background: #fff; color: #564334; font-size: 0.9375rem; font-weight: 700; cursor: pointer; transition: all 0.18s ease; }
        .denuncia-option-button:hover { border-color: rgba(255,140,0,0.55); color: #904d00; transform: translateY(-1px); }
        .denuncia-option-button.active { border-color: rgba(255,140,0,0.65); background: #fff7ed; color: #904d00; box-shadow: 0 8px 18px rgba(234,88,12,0.08); }
      ...
      <div className="denuncia-root">
        ...
        <nav style={{ position: 'fixed', top: 0, width: '100%', zIndex: 50, backgroundColor: 'rgba(255,255,255,0.8)', backdropFilter: 'blur(20px)', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }} className="hidden-mobile">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', padding: '1rem 1.5rem', maxWidth: '80rem', margin: '0 auto' }}>
            ...
            <a
              href="/denuncias/login"
              style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', backgroundColor: '#fff7ed', border: '1px solid rgba(234,88,12,0.18)', color: '#ea580c', borderRadius: '0.75rem', padding: '0.625rem 1rem', fontFamily: 'Inter, sans-serif', fontSize: '0.875rem', fontWeight: 700, textDecoration: 'none', boxShadow: '0 8px 18px rgba(234,88,12,0.08)' }}
            >
          ...
          {sucesso && (
            <div style={{ maxWidth: '42rem', margin: '4rem auto', textAlign: 'center' }}>
              <div className="denuncia-shadow-ambient" style={{ backgroundColor: '#ffffff', borderRadius: '1.5rem', padding: '3rem 2rem', position: 'relative', overflow: 'hidden' }}>
                ...
                <button
                  onClick={() => { setSucesso(null); setForm({ vinculo_grupo:'', tipo_ocorrencia:'', data_ocorrencia:'', local_ocorrencia:'', descricao:'', envolvidos:'', testemunhas:'', nome:'', email:'', telefone:'' }); setFiles([]); setTermos(false); setIdentificar(false); }}
              ...
              <div className="denuncia-shadow-ambient" style={{ backgroundColor: '#ffffff', borderRadius: '1.5rem', padding: '2rem', position: 'relative', overflow: 'hidden' }}>
                ...
                <form style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }} onSubmit={handleSubmit}>
                  ...
                  <fieldset className="denuncia-fieldset">
                    ...
                    <div>
                      <span className="denuncia-label">Qual o seu vínculo com o Grupo Muller?</span>
                      <div className="denuncia-option-grid" role="radiogroup" aria-label="Qual o seu vínculo com o Grupo Muller?">
                        {VINCULO_OPTIONS.map((option) => (
                          <button
                            key={option.value}
                            type="button"
                            className={`denuncia-option-button${form.vinculo_grupo === option.value ? ' active' : ''}`}
                            aria-pressed={form.vinculo_grupo === option.value}
                            onClick={() => setForm((current) => ({ ...current, vinculo_grupo: option.value }))}
                          >
                            {option.label}
                          </button>
                        ))}
