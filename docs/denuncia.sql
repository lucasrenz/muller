...
-- =========================================================
-- 2) FUNÇÃO PARA GERAR PROTOCOLO
-- Formato: DEN-2026-000001
-- =========================================================
...
begin
  ...
  -- garante unicidade caso haja concorrência
  while exists (
    select 1
    from public.denuncias
...
-- =========================================================
-- 3) FUNÇÃO PARA GERAR CÓDIGO DE ACESSO
-- Exemplo: A7K9P2QX
-- =========================================================
...
create table if not exists public.denuncias (
  ...
  -- se quiser permitir denúncia identificada
...
-- =========================================================
-- 6) TABELA DE PUBLICAÇÕES AO DENUNCIANTE
-- =========================================================
...
-- =========================================================
-- 9) TRIGGERS DE CRIAÇÃO DA DENÚNCIA
--
-- Problema: um BEFORE INSERT não pode inserir em tabelas filhas
-- via FK pois o registro pai ainda não existe.
-- Solução: dois triggers separados:
--   a) BEFORE INSERT → apenas modifica NEW (campos da própria linha)
--   b) AFTER INSERT  → insere nos filhos (denuncia_logs, denuncia_publicacoes)
-- =========================================================

-- 9a) BEFORE: preenche os campos da própria linha
create or replace function public.set_campos_criacao_denuncia()
returns trigger
language plpgsql
as $$
begin
  new.resposta_publica_atual := 'Sua denúncia foi recebida com sucesso e será analisada pela equipe responsável.';
...
-- 9b) AFTER: insere nos filhos (pai já existe na tabela)
...
begin
  ...
  values (
    ...
    case
      when new.anonimo then 'Denúncia anônima'
...
-- =========================================================
-- 10) FUNÇÃO AUXILIAR PARA PUBLICAR RESPOSTA OFICIAL
-- Já grava em publicações, atualiza denúncia e cria log
-- =========================================================
...
-- =========================================================
-- 11) FUNÇÃO AUXILIAR PARA ADICIONAR NOTA INTERNA
-- =========================================================
...
-- Leitura e edição apenas para usuários autenticados (gestores RH/financeiro)
...
-- denuncia_anexos: anon pode inserir (logo após criar a denúncia)
...
-- denuncia_logs: apenas leitura autenticada (escrita feita via trigger/função SECURITY DEFINER)
...
-- denuncia_publicacoes: leitura autenticada; anon não acessa
...
-- =========================================================
-- 13) FUNÇÕES DE TRIGGER COM SECURITY DEFINER
-- As funções de trigger que escrevem em tabelas filhas precisam
